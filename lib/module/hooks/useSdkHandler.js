...
import { ZoomVideoSdk } from '../native/ZoomVideoSdk';
...
export function useSdkHandler() {
  ...
  const sdkAudioSettingHelperHandler = useRef(new ZoomVideoSdkAudioSettingHelper());
  ...
  const sdkTestAudioDeviceHelper = useRef(new ZoomVideoSdkTestAudioDeviceHelper());
  const sdkLiveTranscriptionHelper = useRef(new ZoomVideoSdkLiveTranscriptionHelper());
  const sdkRemoteCameraControlHelper = useRef(new ZoomVideoSdkRemoteCameraControlHelper());
  const sdkVirtualBackgroundHelper = useRef(new ZoomVideoSdkVirtualBackgroundHelper());
  const sdkCRCHelper = useRef(new ZoomVideoSdkCRCHelper());
  ...
  const joinSession = useCallback(joinConfig => {
  ...
  const acceptRecordingConsent = useCallback(() => {
    return sdkHandler.current.acceptRecordingConsent();
  }, []);
  const declineRecordingConsent = useCallback(() => {
    return sdkHandler.current.declineRecordingConsent();
  }, []);
  const getRecordingConsentType = useCallback(() => {
